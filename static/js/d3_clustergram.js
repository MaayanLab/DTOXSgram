...
function Search(params, nodes, prop) {
  ...
  function highlight_entity(search_term) {
    ...
    d3.selectAll('.row_label_text')
      ...
      .select('rect')
      .style('opacity', 1)
      // !! DTOXSgram hack - show highlight rect  
      .attr('x','-10px')
      .attr('width','10px');
  }
...
function Labels(){
  ...
  function make_rows(params, row_nodes, reorder){
    ...
    if (params.bounding_width_max.row == 0){
      params.bounding_width_max.row = 0.75;
    }
...
function Viz(config) {
  ...
  function make(config) {
    ...
    params.zoom = d3.behavior
      .zoom()
      // DTOXSgram safari hack
      // .scaleExtent([1.0, params.viz.real_zoom * params.viz.zoom_switch])
      .scaleExtent([1.001, params.viz.real_zoom * params.viz.zoom_switch])
      .on('zoom', zoom.zoomed);
  ...
  function reset_visualization_size() {
    ...
    //////////////////////////////
    // !! hack
    params.zoom.scale(1.01).translate(
...
viz.two_translate_zoom(0,0,1.0001);
